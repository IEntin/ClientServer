...
COMMUNICATION_TYPE=$(grep "CommunicationType" $SERVER_DIR/client_bin/ProgramOptions.json)
...
COMMUNICATION_TYPE=$(grep "CommunicationType" $SERVER_DIR/../PrjClient2/ProgramOptions.json)
if [[ "$COMMUNICATION_TYPE" == *"TCP"* ]]
then
...
SERVER_PID=$!
echo $SERVER_PID
/bin/cp -f $SERVER_DIR/client_bin/client $SERVER_DIR/../PrjClient2
sleep 2

# Start tcp or fifo client.
( cd $SERVER_DIR/client_bin; $SERVER_DIR/client_bin/profile_client.sh& )
...
# profile_client.sh, ProgramOptions.json, and the link to SERVER_DIR/data directory.
...
gprof -b $SERVER_DIR/client_bin/client \
  $SERVER_DIR/client_bin/gmon.out > $SERVER_DIR/$FIRST_CLIENT_PROFILE
gprof -b $SERVER_DIR/../PrjClient2/client $SERVER_DIR/../PrjClient2/gmon.out > \
  $SERVER_DIR/$SECOND_CLIENT_PROFILE
