...
const CryptoPP::OID CryptoNS::_curve = CryptoPP::ASN1::secp256r1();

CryptoNS::CryptoNS(const CryptoPP::SecByteBlock& pubB) :
  _dh(_curve),
  _privKey(_dh.PrivateKeyLength()),
  _pubKey(_dh.PublicKeyLength()),
  _generatedKeyPair(generateKeyPair(_dh, _privKey, _pubKey)),
...
void CryptoNS::showKeyIv(const CryptoPP::SecByteBlock& iv) {
...
std::string_view CryptoNS::encrypt(bool encrypt, std::string_view data) {
...
std::string_view CryptoNS::decrypt(std::string_view data) {
...
bool CryptoNS::handshake(const CryptoPP::SecByteBlock& pubAreceived) {
