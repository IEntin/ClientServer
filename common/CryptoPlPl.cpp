...
CryptoPlPl::CryptoPlPl(std::string_view encodedPeerAesPubKey,
  ...
  _signatureWithPubKeySign(&*signatureWithPubKey.cbegin(), signatureWithPubKey.size()) {
  ...
  peerAesPubKey.Assign(&*decoded.cbegin(), decoded.size());
  ...
  std::string signature(&*_signatureWithPubKeySign.cbegin(), RSA_KEY_SIZE >> 3);
...
void CryptoPlPl::decrypt(std::string& buffer, std::string& data) {
  ...
  if (isEncrypted(data)) {
    ...
    CryptoPP::SecByteBlock
      iv(std::bit_cast<CryptoPP::byte*>(&*data.cbegin() + data.size() - CryptoPP::AES::BLOCKSIZE),
...
void CryptoPlPl::signMessage() {
  ...
  CryptoPP::StringSource ss(&*_msgHash.cbegin(),
  true,
...
bool CryptoPlPl::decodeRsaPublicKey(std::string_view serializedKey,
  ...
  try {
    CryptoPP::StringSource pubKeySource({ &*serializedKey.cbegin(), serializedKey.size() }, true);
...
bool CryptoPlPl::verifySignature(std::string_view signature) {
  ...
  _verified = verifier.VerifyMessage(std::bit_cast<CryptoPP::byte*>(&*_msgHash.cbegin()), _msgHash.length(),
				     std::bit_cast<CryptoPP::byte*>(&*signature.cbegin()), signature.length());
  if (!_verified)
...
std::string CryptoPlPl::sha256_hash(std::string_view message) {
  ...
  std::string digest;
  hash.Update(std::bit_cast<unsigned char*>(&message.front()), message.size());
...
void CryptoPlPl::destroySecretData() {
  CryptoPP::memset_z(&_msgHash.front(), 0, _msgHash.size());
  _rsaPrivKey.GenerateRandomWithKeySize(_rng, RSA_KEY_SIZE);
  CryptoPP::memset_z(&_serializedRsaPubKey.front(), 0, _serializedRsaPubKey.size());
  CryptoPP::memset_z(&_signatureWithPubKeySign.front(), 0, _signatureWithPubKeySign.size());
}
...
std::vector<unsigned char> CryptoPlPl::base64_decode(std::string_view encoded) {
  ...
  try {
    ...
    decoder.Put(std::bit_cast<CryptoPP::byte*>(&encoded.front()), encoded.size());
    decoder.MessageEnd();
  }
