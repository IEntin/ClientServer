...
BOOST_INCLUDES := /usr/local/boost_1_82_0
CRYPTOPP_INCLUDES := /usr/local/include/cryptopp

INCLUDES := -I$(COMMONDIR) -I. -I$(BOOST_INCLUDES) -I$(CRYPTOPP_INCLUDES) -I$(CLIENTSRCDIR) \
...
$(PCH) : $(ALLH)
	$(CXX) -g -x c++-header $(CPPFLAGS) -I$(BOOST_INCLUDES) -I$(CRYPTOPP_INCLUDES) $(ALLH) -o $@
...
server : $(COMMONOBJ) $(SERVEROBJ)
	$(CXX) -o $(SERVERBIN) $(SERVEROBJ) $(COMMONOBJ) $(CPPFLAGS) -pthread $(CRYPTOLIB)
...
$(CLIENTBIN) : $(COMMONOBJ) $(CLIENTOBJ)
	$(CXX) -o $@ $(CLIENTOBJ) $(COMMONOBJ) $(CPPFLAGS) -pthread $(CRYPTOLIB)
...
$(CLIENTFILTEREDOBJ) $(CPPFLAGS) -pthread $(CRYPTOLIB)
...
gmon.out */gmon.out *.gcov *.gcno *.gcda *~ */*~ .server_running *.sec
