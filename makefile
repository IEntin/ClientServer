...
$(PCH) : $(ALLH) $(CRYPTOPPLIB) $(BOTANCRYPTOLIB) $(CRYPTOSODIUMLIB)
	$(CXX) -g -x c++-header $(CPPFLAGS) -I$(BOOST_INCLUDES) $(ALLH) -o $@

-include $(BUILDDIR)/*.d

$(BUILDDIR)/%.o : %.cpp $(PCH) $(CRYPTOPPLIB) $(BOTANCRYPTOLIB) $(CRYPTOSODIUMLIB)
...
serverX : $(COMMONOBJ) $(BUSINESSOBJ) $(POLICYOBJ) $(SERVEROBJ) $(CRYPTOPPLIB) $(BOTANCRYPTOLIB) $(CRYPTOSODIUMLIB)
	$(CXX) -o $(SERVERBIN) $(SERVEROBJ) $(COMMONOBJ) $(BUSINESSOBJ) $(POLICYOBJ) \
$(CPPFLAGS) -pthread $(CRYPTOPPLIB) $(BOTANCRYPTOLIB) $(CRYPTOSODIUMLIB) -llz4 -lsnappy -lzstd
...
$(CLIENTBIN) : $(COMMONOBJ) $(CLIENTOBJ) $(CRYPTOPPLIB) $(BOTANCRYPTOLIB) $(CRYPTOSODIUMLIB)
	$(CXX) -o $@ $(CLIENTOBJ) $(COMMONOBJ) $(CPPFLAGS) -pthread $(CRYPTOPPLIB) $(BOTANCRYPTOLIB) $(CRYPTOSODIUMLIB) -llz4 -lsnappy -lzstd

TESTSRC := $(wildcard $(TESTSRCDIR)/*.cpp)
TESTOBJ := $(patsubst $(TESTSRCDIR)/%.cpp, $(BUILDDIR)/%.o, $(TESTSRC))

$(TESTBIN) : $(COMMONOBJ) $(BUSINESSOBJ) $(POLICYOBJ) $(SERVERFILTEREDOBJ) $(CLIENTFILTEREDOBJ) $(TESTOBJ) $(CRYPTOPPLIB) $(BOTANCRYPTOLIB) $(CRYPTOSODIUMLIB)
	$(CXX) -o $@ $(TESTOBJ) -lgtest $(COMMONOBJ) $(BUSINESSOBJ) $(POLICYOBJ) $(SERVERFILTEREDOBJ) \
$(CLIENTFILTEREDOBJ) $(CPPFLAGS) -pthread $(CRYPTOPPLIB) $(BOTANCRYPTOLIB) $(CRYPTOSODIUMLIB) -llz4 -lsnappy -lzstd
